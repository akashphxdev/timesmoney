...
export const Testimonials = ({ testimonials }: { testimonials: Testimonial[] }) => {
  ...
  const max = testimonials.length - SHOW;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="overflow-hidden">
          <div
            ...
            style={{ transform: `translateX(-${cur * (100 / testimonials.length)}%)`, width: `${(testimonials.length / SHOW) * 100}%` }}
          >
            {testimonials.map((item) => (
              <div
                ...
                className="px-3"
              >
                <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-brand-teal/10 hover:-translate-y-1.5 transition-all duration-500 flex flex-col gap-5 h-full">
        ...
        <div className="flex items-center justify-center gap-2 mt-10">
          <button onClick={() => resetTimer(cur - 1)} className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-slate-400 hover:border-brand-teal hover:text-brand-teal transition-all">
            ←
          </button>
