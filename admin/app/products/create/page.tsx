...
// ✅ FIX: SubCategory mein category object add kiya (blog jaisa)
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={add}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 text-green-600 text-xs font-semibold rounded-lg hover:bg-green-100 transition-colors"
        >
...
export default function CreateProductPage() {
  ...
  // ✅ FIX: Saari sub-categories ek baar fetch karenge
  const [allSubCategories, setAllSubCategories] = useState<SubCategory[]>([]);
  // ✅ FIX: Filtered subs — category select hone par filter hogi
  ...
  // ✅ FIX: Dono ek saath fetch karo — blog page jaisa
  useEffect(() => {
    api.get('/categories').then((res) => setCategories(res.data.data)).catch(() => {});
    api.get('/sub-categories').then((res) => setAllSubCategories(res.data.data)).catch(() => {});
  }, []);

  // ✅ FIX: Category change hone par frontend pe filter karo — koi extra API call nahi
  ...
  const updateFormField = <K extends keyof FormField>(
    index: number, key: K, value: FormField[K]
  ) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const cleanEligibility = eligibility.filter((e) => e.trim());

      if (cleanFeatures.length > 0) formData.append('features', JSON.stringify(cleanFeatures));
      if (cleanBenefits.length > 0) formData.append('benefits', JSON.stringify(cleanBenefits));
      if (cleanEligibility.length > 0) formData.append('eligibility', JSON.stringify(cleanEligibility));
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Add a new financial product</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                {providerLogoPreview && (
                  <img src={providerLogoPreview} alt="logo" className="w-16 h-16 object-contain rounded-lg mb-2" />
                )}
                <label className="w-full flex items-center justify-center px-4 py-2.5 text-sm text-gray-500 bg-gray-50 border border-dashed border-gray-300 rounded-xl cursor-pointer hover:bg-gray-100">
                  {providerLogo ? providerLogo.name : 'Choose logo'}
                  <input type="file" accept="image/*" onChange={(e) => {
                    const file = e.target.files?.[0];
                    if (!file) return;
                    setProviderLogo(file);
                    setProviderLogoPreview(URL.createObjectURL(file));
                  }} className="hidden" />
                </label>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                ...
                <input
                  type="url" value={form.applyUrl} placeholder="https://example.com/apply"
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
            ...
            {formFields.length === 0 && (
              <p className="text-xs text-gray-400 text-center py-4">
                No custom fields — click Add Field to add
              </p>
            )}
            ...
            {formFields.map((field, index) => (
              <div key={index} className="border border-gray-200 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => removeFormField(index)} className="text-red-400 hover:text-red-600">
                    <AiOutlineDelete size={16} />
                  </button>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <div className="col-span-2">
                    ...
                    <input
                      type="text" placeholder="e.g. Monthly Income" value={field.label}
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox" id={`required-${index}`} checked={field.required}
                    onChange={(e) => updateFormField(index, 'required', e.target.checked)}
                    className="accent-green-500"
                  />
                  <label htmlFor={`required-${index}`} className="text-xs text-gray-600">Required</label>
                </div>
                {field.type === 'select' && (
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Options (comma separated)</label>
                    <input
                      type="text" placeholder="e.g. Salaried, Self Employed, Business"
                      value={field.options?.join(', ') || ''}
                      onChange={(e) =>
                        updateFormField(index, 'options', e.target.value.split(',').map((o) => o.trim()))
                      }
                      className="w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg outline-none focus:border-green-400 text-gray-800"
                    />
                  </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
            ...
            <div>
              ...
              <input
                type="text" placeholder="e.g. New, Hot, Popular" value={form.badge}
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-3">
            ...
            {imagePreview && (
              <img src={imagePreview} alt="product" className="w-full h-40 object-cover rounded-xl" />
            )}
            <label className="w-full flex items-center justify-center px-4 py-2.5 text-sm text-gray-500 bg-gray-50 border border-dashed border-gray-300 rounded-xl cursor-pointer hover:bg-gray-100">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
            ...
            {/* ✅ FIX: filteredSubs use kar raha hai ab */}
            {form.categoryId && (
              <div>
                <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1.5 block">
                  Sub Category
