...
export default function EditAdPage() {
    ...
    api
      ...
      .then((res) => {
        ...
        setForm({
          ...
          size: ad.size || 'BANNER_300x250',
          customWidth: ad.customWidth ? String(ad.customWidth) : '',
          customHeight: ad.customHeight ? String(ad.customHeight) : '',
          link: ad.link || '',
          active: ad.active ?? true,
          sortOrder: String(ad.sortOrder ?? 0),
          startDateTime: toDatetimeLocal(ad.startDateTime),
          endDateTime: toDatetimeLocal(ad.endDateTime),
        });
  ...
  const validate = () => {
    ...
    if (!existingImage && !newImage) errors.image = 'Ad image is required';
    if (form.size === 'CUSTOM') {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.patch(`/ads/${id}`, formData, {
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Update advertisement details</p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
            ...
            <div>
              ...
              <div className={`grid grid-cols-3 gap-2 p-3 border rounded-xl ${fieldErrors.pages ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-gray-50'}`}>
                {AD_PAGES.map((page) => (
                  <label key={page.value} className="flex items-center gap-2 cursor-pointer group">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-xs text-gray-700 group-hover:text-green-600">{page.label}</span>
                  </label>
                ))}
              </div>
                ...
                {AD_POSITIONS.map((pos) => (
                  <label key={pos.value} className="flex items-center gap-2 cursor-pointer group">
                    <input
                      ...
                      className="w-4 h-4 accent-green-500 rounded"
                    />
                    <span className="text-xs text-gray-700 group-hover:text-green-600">{pos.label}</span>
                  </label>
                ))}
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-3">
            ...
            {(imagePreview || existingImage) && (
              <img
                src={imagePreview || `${process.env.NEXT_PUBLIC_SERVER_URL}${existingImage}`}
                alt="ad"
            ...
            <label className={`w-full flex items-center justify-center px-4 py-2.5 text-sm text-gray-500 border border-dashed rounded-xl cursor-pointer hover:bg-gray-100 ${fieldErrors.image ? 'border-red-400 bg-red-50' : 'bg-gray-50 border-gray-300'}`}>
              {newImage ? newImage.name : 'Change image'}
              <input type="file" accept="image/*" onChange={handleImage} className="hidden" />
            </label>
            <FieldError field="image" />
            {existingImage && !newImage && (
              <p className="text-xs text-gray-400">Current image will be kept if no new image selected.</p>
            )}
          </div>
